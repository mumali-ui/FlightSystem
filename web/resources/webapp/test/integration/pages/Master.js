				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("FLIGHT") === oObject.FLIGHT;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].FLIGHT;
										} else {
											sCurrentId = aItemsNotInTheList[0].FLIGHT;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Connections", {
											FLIGHT : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("FLIGHT"),
							title: oBindingContext.getProperty("FR_CITY")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "Connections (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'Connections (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("FR_CITY");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all Connections to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "Connections (" + iExpectedLength + ")"}),
									success : function () {
